...
document.addEventListener('smwe:ready', (event) => {
    ...
    (() => {
        ...
        function renderUI() {
            ...
            const hasLink = currentMainFile && sidecarLinks[currentMainFile];
            const filePath = hasLink ? sidecarLinks[currentMainFile] : '未关联附属文件';
            
            container.innerHTML = `
                <div id="sidecar-header">
                    <h4>★ 附属文件属性</h4>
                    <div id="sidecar-controls">
                        <button id="sidecar-link-btn" class="btn btn-sm">${hasLink ? '更改' : '关联'}文件</button>
                        ${hasLink ? '<button id="sidecar-unlink-btn" class="btn btn-sm btn-danger">解除关联</button>' : ''}
                    </div>
                </div>
                <div id="sidecar-path-display" title="点击可尝试在系统中打开">${filePath}</div>
                <div id="sidecar-content"><div class="placeholder">请先关联一个文件。</div></div>
            `;

            document.getElementById('sidecar-link-btn').addEventListener('click', linkFile);
            if (hasLink) {
                document.getElementById('sidecar-unlink-btn').addEventListener('click', unlinkFile);
                document.getElementById('sidecar-path-display').addEventListener('click', () => api.eel.open_external_file(filePath)());
                loadAndDisplaySidecarContent(filePath);
            }
        }
        ...
        async function loadAndDisplaySidecarContent(filePath) {
            const contentEl = document.getElementById('sidecar-content');
            contentEl.innerHTML = '正在加载附属文件...';
            const result = await api.eel.read_sidecar_file_content(filePath)();
        ...
        const pluginAPI = {
            async getSidecarDataFor(filename) {
                const filePath = sidecarLinks[filename];
